...
extension GroupProfileTableViewController {
  ...
  @objc func createGroupChat() {
    ...
    chatCreatingGroup.notify(queue: DispatchQueue.main, execute: {
      ...
      self.sendInformatoinMessage(chatID: chatID, membersIDs: membersIDs)
...
extension GroupProfileTableViewController {
  ...
  func sendInformatoinMessage(chatID: String?, membersIDs: ([String], [String:AnyObject])) {
    ...
    let values: [String: AnyObject] = ["messageUID": childRef.key as AnyObject, "toId": toId as AnyObject, "status": defaultMessageStatus as AnyObject , "seen": false as AnyObject, "fromId": fromId as AnyObject, "timestamp": timestamp, "text": "New group has been created" as AnyObject, "isInformationMessage": true as AnyObject]
  ...
  func updateLastMessageForParticipants(chatID: String?, participantsIDs: ([String], [String:AnyObject]), messageID: String) {
    guard let conversationID = chatID else { return }
      for memberID in participantsIDs.0 {
  ...
  func incrementBadgeForReciever(conversationID: String?, participantsIDs: ([String], [String:AnyObject])) {
    guard let currentUserID = Auth.auth().currentUser?.uid, let conversationID = conversationID else { return }
    for participantID in participantsIDs.0 {
